...
\chapter{Handbuch}
Dieses System ist einfach benutzbar und man muss kein technologisches Wissen haben, um das System zu benutzen. Das System besteht aus zwei Teilen: 
\begin{itemize}
	\item Einloggen bzw. Erkennung 
	\item Registrierung 
\end{itemize}
Damit das System wei{\ss}, welche Funktion ausgef\"uhrt wird, gibt es zwei Taster, einen f\"ur jede Funktion. Diese Taster sind durch die Namensbezeichnung, die sie haben, erkennbar. Wenn jemand sich einloggen bzw. registrieren will, muss der Taster der jeweiligen Funktionen gedrückt werden. \\ \\ 
\textbf{Einloggen} \\
Der Benutzer kann sich selbst einloggen, ohne die Hilfe des Administrators. Nach dem Dr\"ucken des Tasters muss der Benutzer die erforderlichen Daten eingeben. Danach muss er vor der Kamera stehen. Nachdem das System ein Bild von ihm gemacht hat, wird \"uberpr\"uft, ob dieser Benutzer schon registriert ist. Am Ende wird an Bildschirm gezeigt, ob der Benutzer eingeloggt ist oder nicht. Als Hilfe gibt es auch die LED, die wenn alles in Ordnung ist grün leuchtet, sonst leuchtet sie rot, als Fehlermeldung. \\ \\
\textbf{Registrierung} \\
Immer wenn ein neuer Benutzer registriert wird, muss ein Administrator dabei sein. Die Aufgabe des Administrators ist die Kontrolle. Nach dem Dr\"ucken des Tasters m\"ussen Informationen von dem Benutzer eingegeben werden. Das System zeigt am Bildschirm, was der Benutzer eingeben muss. Als letzter Schritt muss der Benutzer vor der Kamera stehen bleiben, damit ein Bild von ihm gemacht wird. Am Ende wird am Bildschirm gezeigt, ob der Benutzer registriert wurde oder nicht. Als Hilfe gibt es auch die LED, die wenn alles in Ordnung ist gr\"un leuchtet, sonst leuchtet sie rot, als Fehlermeldung. \\ \\ 
\textbf{Bemerkungen}\\
Damit das System ohne Probleme funktioniert, muss man auf ein paar Dinge achten. Es gibt viele Kleidungsst\"ucke bzw. Accessoires, die das Registrieren bzw. Einloggen schwierig oder fast unmöglich machen. Deshalb soll ein Benutzer vor der Kamera:  
\begin{itemize}
	\item Keine Brille tragen. 
	\item Stehen bleiben und sich nicht bewegen. 
	\item Keine Mütze tragen. 
	\item Keine Maske tragen. 
\end{itemize}
\textbf{Einrichten des Systems}\\
Das System ist einfach und benutzerfreundlich zum Nutzen und Ausf\"uhren. Alle wichtige Information, Verzeichnisse und Dateien werden lokal in einem Raspberry Pi gespeichert. \\ \\
\textbf{Datenbank}\\
Zum Einrichten der Datenbank gibt es eine einzige Datei, die ausgef\"uhrt werden muss. Die Datei hei{\ss}t Datenbank, und danach sollte die Datenbank im Betrieb sein.  \\ \\
\textbf{Kamera und Erkennung}\\
Zum Einrichten der Kameras und des Erkennungssystems gibt es eine einzige Datei, die ausgef\"uhrt werden muss. Die Datei hei{\ss}t Erkennung, und danach ist das System im Betrieb. Abh\"angig von dem, was der Administrator machen will, stehen die Taster nun zur Verf\"ugung. 